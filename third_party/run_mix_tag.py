...
def train(args, train_dataset, mix_dataset, model, tokenizer, labels, pad_token_label_id, lang2id=None):
  """Train the model."""
  ...
  #mix_train_sampler = RandomSampler(mix_dataset) if args.local_rank == -1 else DistributedSampler(mix_dataset)
  #mix_train_dataloader = DataLoader(mix_dataset, sampler=mix_train_sampler, batch_size=args.train_batch_size)
  ...
  for _ in train_iterator:
    ...
    #mix_epoch_iterator = tqdm(mix_train_dataloader, desc="Iteration", disable=args.local_rank not in [-1, 0])
    cur_epoch += 1
    #for step, batch in enumerate(zip(epoch_iterator, mix_epoch_iterator)):
    for step, batch in enumerate(epoch_iterator):
      batch, mix_batch = batch
      model.train()
      if args.tau > 0:
        ...
        mix_batch = tuple(t.to(args.device) for t in mix_batch if t is not None)
      ...
      mix_loss = mix_outputs[0]
      mix_seq_out = mix_outputs[-1]

      loss = 0.8*loss + 0.2*mix_loss
      if args.mix_weight > 0:
        mix_tau = random.uniform(0.1, 0.5)
        mixup_outputs = model.forward_mix(seq1=seq_out, seq2=mix_seq_out, labels1=inputs["labels"], labels2=mix_inputs["labels"], attention_mask1=inputs["attention_mask"], attention_mask2=mix_inputs["attention_mask"], tau=mix_tau)

        loss = loss + args.mix_weight*mixup_outputs[0]
      if args.n_gpu > 1:
...
def main():
  ...
  parser.add_argument("--mix_langs", default="en", type=str)
  ...
  parser.add_argument("--mix_weight", default=0, type=float)
  ...
  if args.do_train:
    ...
    mix_dataset = load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode="train", lang=args.mix_langs, lang2id=lang2id, few_shot=1000)
    global_step, tr_loss = train(args, train_dataset, mix_dataset, model, tokenizer, labels, pad_token_label_id, lang2id)
