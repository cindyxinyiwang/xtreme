...
TGT=${3:-xquad}
GPU=${4:-0}
DATA_DIR=${5:-"$SCRATCH/download/"}
OUT_DIR=${6:-"$SCRATCH/outputs/"}
...
for SEED in 1;
do
  MODEL_PATH=$OUT_DIR/$SRC/${MODEL}_LR${LR}_EPOCH${NUM_EPOCHS}_maxlen${MAXL}_mbped${BPE_DROP}_kl${KL}_klt${KL_T}_kltb${KL_TB}_kltg${KL_TG}_klsg${KL_SG}_rloss${RWEIGHT}_dloss${DWEIGHT}_s${SEED}
  mkdir -p $MODEL_PATH
  ...
  bash scripts/predict_qa_psc.sh $MODEL $MODEL_TYPE $MODEL_PATH xquad $GPU $DATA_DIR
  bash scripts/predict_qa_psc.sh $MODEL $MODEL_TYPE $MODEL_PATH mlqa $GPU $DATA_DIR
  bash scripts/eval_qa_psc.sh $MODEL_PATH
done
